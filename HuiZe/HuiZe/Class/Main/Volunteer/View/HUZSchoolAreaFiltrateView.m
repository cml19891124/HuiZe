...
@interface HUZSchoolAreaFiltrateView ()<UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout>
@property(nonatomic,strong)UICollectionView *collectView;
@property(nonatomic,strong)NSArray *sectionArr;

@end

@implementation HUZSchoolAreaFiltrateView
...
-(void)loadDataWithSectionArr:(NSArray*)sectionArr array:(NSArray*)array {
    ...
    for (int i = 0; i < sectionArr.count; i ++) {
        ...
        for(int i = 0;i<array.count;i++){
            ...
            HUZProvince *model = array[i];
//            model.isSelected = @"0";
            dic[@"name"]= model.title;
            dic[@"id"]= model.Id;//[NSString stringWithFormat:@"%d",i];
