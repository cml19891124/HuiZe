...
/** 输入弹窗 */
@property (nonatomic,strong) HUZInputView *inputView;
/** 敬请期待 */
@property (nonatomic,strong) HUZAwaitView *awaitView;
...
- (nullable instancetype)initWithPopStyle:(ZJAnimationPopStyle)popStyle dismissStyle:(ZJAnimationDismissStyle)dismissStyle{
    self = [super init];
    if (self) {
...
- (NSTimeInterval)getPopDefaultDuration:(ZJAnimationPopStyle)animationPopStyle
{
    ...
    if (animationPopStyle == ZJAnimationPopStyleNO) {
        defaultDuration = 0.2f;
               ...
               animationPopStyle == ZJAnimationPopStyleShake) {
...
/// 显示输入弹窗
- (void)popInputAlertWithTitle:(NSString *)title placeholder:(NSString *)placeholder{
    _inputView = nil;
    _inputView = [[HUZInputView alloc] initWithFrame:self.bounds];
    _inputView.labTitle.text = title;
    _inputView.tfInput.placeholder = placeholder;
    [_inputView.tfInput becomeFirstResponder];
    [_inputView.btnCancel addTarget:self action:@selector(cancel) forControlEvents:UIControlEventTouchUpInside];
    [_inputView.btnConfirm addTarget:self action:@selector(confirmInput) forControlEvents:UIControlEventTouchUpInside];
    _customView = _inputView;
    ...
    [_inputView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.contentView).offset(AutoDistance(172));
        make.centerX.equalTo(self.contentView);
        make.size.mas_equalTo(CGSizeMake(AutoDistance(278), AutoDistance(152)));
    }];
    [self pop];
}

/// 显示提示弹窗
- (void)popAwaitAlertWithTitle:(NSString *)title cancel:(NSString *)cancelStr confirm:(NSString *)confirmStr{
    _awaitView = nil;
    _awaitView = [[HUZAwaitView alloc] initWithFrame:self.bounds];
    [_awaitView.btnConfirm addTarget:self action:@selector(confirm) forControlEvents:UIControlEventTouchUpInside];
    [_awaitView.btnCancel addTarget:self action:@selector(cancel) forControlEvents:UIControlEventTouchUpInside];
    _awaitView.labTitle.text = title;
    [_awaitView.btnCancel setTitle:cancelStr forState:UIControlStateNormal];
    [_awaitView.btnConfirm setTitle:confirmStr forState:UIControlStateNormal];
    _customView = _awaitView;
    ...
    [self.contentView addSubview:_awaitView];
    [_awaitView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.contentView).offset(AutoDistance(190));
        make.centerX.equalTo(self.contentView);
        make.size.mas_equalTo(CGSizeMake(AutoDistance(278), AutoDistance(152)));
    }];
    if (DRStringIsEmpty(cancelStr)) {
        _awaitView.btnCancel.hidden = YES;
        _awaitView.btnConfirm.frame =  CGRectMake(0, AutoDistance(91), AutoDistance(278), AutoDistance(21));
    }
    [self pop];
}
...
- (void)confirmInput{
    ...
    if (self.inputViewBlock) {
        self.inputViewBlock(_inputView.tfInput.text);
    }
}
