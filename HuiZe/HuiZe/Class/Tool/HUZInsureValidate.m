...
#import "HUZInsureValidate.h"

@implementation HUZInsureValidate
// 获取高考日期
+ (NSString *)getGkTimeStamp{
    ...
    NSString *thisYearString=[dateformatter1 stringFromDate:senddate];
    
    /// 获取当年高考当天时间戳
    NSString *gkTimeStr = [NSString stringWithFormat:@"%@-06-07",thisYearString];
    NSDateFormatter *formatter2 = [[NSDateFormatter alloc] init];
    [formatter2 setDateStyle:NSDateFormatterMediumStyle];
    [formatter2 setTimeStyle:NSDateFormatterShortStyle];
    [formatter2 setDateFormat:@"YYYY-MM-dd"];
    /// 获取下一年高考时间戳
    NSString *nextGkTimeStr = [NSString stringWithFormat:@"%d-06-07",[thisYearString integerValue] + 1];
    
    NSDate *datenow = [NSDate date]; /// 现在时间
    NSDate *date = [formatter2 dateFromString:gkTimeStr]; /// 当年高考时间
    NSDate *nextDate = [formatter2 dateFromString:nextGkTimeStr]; /// 下一年高考时间
    
    long timeNow = (long)[datenow timeIntervalSince1970];
    long gkTime = (long)[date timeIntervalSince1970];
    long nextGkTime = (long)[nextDate timeIntervalSince1970];
    
    if (gkTime - timeNow < 0) { /// 今年高考结束，取下一年高考时间
        return [NSString stringWithFormat:@"%ld",(nextGkTime-timeNow)/86400 + 1];
    }else{
        return [NSString stringWithFormat:@"%ld",(gkTime-timeNow)/86400 + 1];
    }
...
{
    NSString *newStr;
    NSString *deleteSpaceStr = [HUZInsureValidate deleteWhiteSpaceInStr:phoneStr];
    NSString *deleteSpecialStr = [HUZInsureValidate deleteSpecialStr:deleteSpaceStr];
    NSString *deleteSpecialStr1 = [HUZInsureValidate deleteSpecialStr1:deleteSpecialStr];
